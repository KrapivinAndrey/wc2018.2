...
Функция ПсевдоКорабльЦентр()
	ПсевдоКорабльЦентр = Новый Соответствие();
	ПсевдоКорабльЦентр.Вставить("Position", "4/2/0");
	ПсевдоКорабльЦентр.Вставить("Velocity", "0/0/0");
КонецФункции


Процедура Построится(Корабли, Ответ)
	
	Если НеВыполнять = Неопределено Тогда 
		НеВыполнять = Ложь;
	КонецЕсли;
	
	Если  НовыйВектор(Корабли[0]["Position"]).X = 2
		И НовыйВектор(Корабли[0]["Position"]).Y = 4   
		И НовыйВектор(Корабли[0]["Position"]).Z = 0   
		И НовыйВектор(Корабли[1]["Position"]).X = 2
		И НовыйВектор(Корабли[1]["Position"]).Y = 0   
		И НовыйВектор(Корабли[1]["Position"]).Z = 0   
		И НовыйВектор(Корабли[2]["Position"]).X = 4
		И НовыйВектор(Корабли[2]["Position"]).Y = 2   
		И НовыйВектор(Корабли[2]["Position"]).Z = 0   
		И НовыйВектор(Корабли[3]["Position"]).X = 6
		И НовыйВектор(Корабли[3]["Position"]).Y = 0   
		И НовыйВектор(Корабли[3]["Position"]).Z = 0   
		И НовыйВектор(Корабли[4]["Position"]).X = 6
		И НовыйВектор(Корабли[4]["Position"]).Y = 4   
		И НовыйВектор(Корабли[4]["Position"]).Z = 0 
		И НовыйВектор(Корабли[0]["Velocity"]).X = 0  
		И НовыйВектор(Корабли[0]["Velocity"]).Y = 0  
		И НовыйВектор(Корабли[0]["Velocity"]).Z = 0  
		И НовыйВектор(Корабли[1]["Velocity"]).X = 0  
		И НовыйВектор(Корабли[1]["Velocity"]).Y = 0  
		И НовыйВектор(Корабли[1]["Velocity"]).Z = 0  
		И НовыйВектор(Корабли[2]["Velocity"]).X = 0  
		И НовыйВектор(Корабли[2]["Velocity"]).Y = 0  
		И НовыйВектор(Корабли[2]["Velocity"]).Z = 0  
		И НовыйВектор(Корабли[3]["Velocity"]).X = 0  
		И НовыйВектор(Корабли[3]["Velocity"]).Y = 0  
		И НовыйВектор(Корабли[3]["Velocity"]).Z = 0  
		И НовыйВектор(Корабли[4]["Velocity"]).X = 0  
		И НовыйВектор(Корабли[4]["Velocity"]).Y = 0  
		И НовыйВектор(Корабли[4]["Velocity"]).Z = 0  Тогда
	...
	Если НЕ НеВыполнять И (НЕ НовыйВектор(Корабли[0]["Position"]).Y = 4
		ИЛИ НЕ НовыйВектор(Корабли[2]["Position"]).Y = 2 
		ИЛИ НЕ НовыйВектор(Корабли[2]["Position"]).X = 4
		ИЛИ НЕ НовыйВектор(Корабли[4]["Position"]).Y = 4 
		ИЛИ НЕ НовыйВектор(Корабли[4]["Position"]).X = 8)
		Тогда 
		ДобавитьКоманду(Ответ, Ускорение(Корабли[0], НовыйВектор(0,4,0)));
		
		ДобавитьКоманду(Ответ, Ускорение(Корабли[2], НовыйВектор(4,2,0)));
		
		ДобавитьКоманду(Ответ, Ускорение(Корабли[4], НовыйВектор(8,4,0)));
		
		
		
	Иначе
		НеВыполнять = Истина;
		ДобавитьКоманду(Ответ, Ускорение(Корабли[0], НовыйВектор(2,4,0)));
		
		ДобавитьКоманду(Ответ, Ускорение(Корабли[4], НовыйВектор(6,4,0)));
...
Функция ОбщаяЦель()
	...
	БлизкаяКоордината = 30 ;
	БлизкийВраг = Неопределено;
	
	Для Каждого Враг из КораблиПротивника Цикл 
		
		Если НовыйВектор(Враг["Position"]).Z < БлизкаяКоордината Тогда
			БлизкийВраг = Враг; 
			БлизкаяКоордината = НовыйВектор(Враг["Position"]).Z;
		КонецЕсли;
...
Функция ПостроитсяОтносительноСередины(Ответ)
	
	
	Отказ = Ложь;
	Собран = Истина; 

	Если ПсевдоКорабльЦентр = неопределено  ИЛИ ПсевдоКорабльЦентр["Position"] = Неопределено Тогда 
		ПсевдоКорабльЦентр();
	КонецЕсли;

	КоординатыКорабляСередины = НовыйВектор(ПсевдоКорабльЦентр["Position"]);
	
	Для каждого Корабль из Корабли Цикл 
		
		Если Корабль["Id"] = 0  И (КоординатыКорабляСередины.X - 2 < 0 ИЛИ КоординатыКорабляСередины.Y + 2 > 28) Тогда
			Отказ = Истина;
		КонецЕсли;
		Если Корабль["Id"] = 1 И (КоординатыКорабляСередины.X - 2 < 0 ИЛИ КоординатыКорабляСередины.Y - 2 < 0) Тогда 
			Отказ = Истина;
		КонецЕсли;
		Если Корабль["Id"] = 2 Тогда 
			//ДобавитьКоманду(Ответ, Ускорение(Корабль, НовыйВектор(КоординатыКорабляСередины.X , КоординатыКорабляСередины.Y, КоординатыКорабляСередины.Z)));
		КонецЕсли;
		Если Корабль["Id"] = 3 И (КоординатыКорабляСередины.X + 2 > 28 ИЛИ КоординатыКорабляСередины.Y - 2 < 0) Тогда 
			Отказ = Истина;
		КонецЕсли;
		Если Корабль["Id"] = 4 И (КоординатыКорабляСередины.X + 2 > 28 ИЛИ КоординатыКорабляСередины.Y + 2 > 28) Тогда 
			Отказ = Истина;
		КонецЕсли;
	...
	Если Не Отказ Тогда 
		...
		Для Каждого Корабль из Корабли Цикл
			Если Корабль["Id"] = 0
				И НЕ(НовыйВектор(Корабль["Position"]).X = КоординатыКорабляСередины.X - 2 
				И НовыйВектор(Корабль["Position"]).Y = КоординатыКорабляСередины.Y + 2) Тогда 
				Собран = Ложь;
			КонецЕсли;
			...
			Если Корабль["Id"] = 2
				И НЕ(НовыйВектор(Корабль["Position"]).X = КоординатыКорабляСередины.X
				И НовыйВектор(Корабль["Position"]).Y = КоординатыКорабляСередины.Y) Тогда 
				Собран = Ложь;
			КонецЕсли;
	...
	Если Собран ИЛИ Отказ Тогда 
		Возврат Истина;
	КонецЕсли;
	
	
	Для каждого Корабль из Корабли Цикл 
		Если Корабль["Id"] = 0 Тогда
			ДобавитьКоманду(Ответ, Ускорение(Корабль, НовыйВектор(КоординатыКорабляСередины.X - 2 , КоординатыКорабляСередины.Y + 2 , КоординатыКорабляСередины.Z)));
		КонецЕсли;
		Если Корабль["Id"] = 1 Тогда 
			ДобавитьКоманду(Ответ, Ускорение(Корабль, НовыйВектор(КоординатыКорабляСередины.X - 2 , КоординатыКорабляСередины.Y - 2, КоординатыКорабляСередины.Z)));
		КонецЕсли;
		Если Корабль["Id"] = 2 Тогда 
			ДобавитьКоманду(Ответ, Ускорение(Корабль, НовыйВектор(КоординатыКорабляСередины.X , КоординатыКорабляСередины.Y, КоординатыКорабляСередины.Z)));
		КонецЕсли;
		Если Корабль["Id"] = 3 Тогда 
			ДобавитьКоманду(Ответ, Ускорение(Корабль, НовыйВектор(КоординатыКорабляСередины.X + 2 , КоординатыКорабляСередины.Y - 2, КоординатыКорабляСередины.Z)));
		КонецЕсли;
		Если Корабль["Id"] = 4 Тогда 
			ДобавитьКоманду(Ответ, Ускорение(Корабль, НовыйВектор(КоординатыКорабляСередины.X + 2 , КоординатыКорабляСередины.Y + 2, КоординатыКорабляСередины.Z)));
		КонецЕсли;
	КонецЦикла;
...
Функция ПодготовитьОтвет()
	...
	БлизкийВраг = ОбщаяЦель(); // Сравнять Y X 
	БлизкийВрагX = НовыйВектор(БлизкийВраг["Position"]).X;
	БлизкийВрагY = НовыйВектор(БлизкийВраг["Position"]).Y;
	
	//Середина = "4/2/0";
	
	Если Построились = Неопределено ИЛИ Построились = Ложь Тогда 
	 Построится(Корабли, Ответ);
	КонецЕсли;
	Если Построились Тогда 
				
			ВекторУскорения = ПонятьУскорятьсяИлиЗамедлять(Корабли[0], НовыйВектор(БлизкийВрагX,БлизкийВрагY,0));

			Для каждого Корабль из Корабли Цикл 
				Отладка.Добавить("" + Корабль["Id"] + "v - " + ВекторСтрокой(ВекторУскорения));
				ДобавитьКоманду(Ответ, Ускорение_Стандарт(Корабль, ВекторУскорения));
				ДобавитьКоманду(Ответ, Выстрел(Корабль, БлизкийВраг["Position"]));
			КонецЦикла;
...
Функция ПереместитьКорабльПризрак(ВекторУскорения)

	
	
	ПозицияТекущая = НовыйВектор(ПсевдоКорабльЦентр["Position"]) ; // "4/2/0";
	СкоростьТекущая = НовыйВектор(ПсевдоКорабльЦентр["Velocity"]);  // 0

	СкоростьПоОкончаниюX = СкоростьТекущая.X + ВекторУскорения.X;
	СкоростьПоОкончаниюY = СкоростьТекущая.Y + ВекторУскорения.Y;
	СкоростьПоОкончаниюZ = СкоростьТекущая.Z + ВекторУскорения.X;


	x = ПозицияТекущая.X + СкоростьПоОкончаниюX ;
	y = ПозицияТекущая.Y + СкоростьПоОкончаниюY ;
	z = ПозицияТекущая.Z + СкоростьПоОкончаниюZ ;

	Position = НовыйВектор(СкоростьПоОкончаниюX, СкоростьПоОкончаниюY, СкоростьПоОкончаниюZ);
	Velocity = НовыйВектор(СкоростьПоОкончаниюX, СкоростьПоОкончаниюY, СкоростьПоОкончаниюZ);

	ПсевдоКорабльЦентр.Вставить("Position", ВекторСтрокой(Position));
	ПсевдоКорабльЦентр.Вставить("Velocity", ВекторСтрокой(Velocity));
...
Функция ПонятьУскорятьсяИлиЗамедлять(Корабль, Точка)

	СтруктураОтвета = НовыйПараметрыУскорения(Корабль, Точка);
	ВекторУскорения = НовыйВектор(СтруктураОтвета.Vector); 

	Возврат ВекторУскорения;

КонецФункции


Функция Ускорение_Стандарт(Корабль, КоординатыЦели)
	
	Результат = Новый Структура();
	Результат.Вставить("Command",        "ACCELERATE");
	Результат.Вставить("Parameters",      НовыйПараметрыУскорение_Стандарт(Корабль, НовыйВектор(КоординатыЦели)));
...
Функция НовыйПараметрыУскорение_Стандарт(Корабль, Вектор)

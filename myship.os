...
Процедура ВыполнитьСценарий()
	...
	// 		КомандаПеремещения = Летим(Корабль, НачальнаяПозицияКорабля(Корабль), Истина);
	// 	Иначе				
	// 		КомандаПеремещения = Летим(Корабль, ЦельПеремещения["Position"], Истина);
	// 	КонецЕсли;
...
Процедура ВыполнитьСценарийДрафт()
	...
	СостояниеDraftOption = ПарсерJSON.ПрочитатьJSON(ВходныеДанные);
	РазмерКарты = СостояниеDraftOption["MapSize"];	
	//ИнициализироватьОкружениеDraftOption(СостояниеDraftOption);
	//Отладка.Добавить(ВходныеДанные);
	МассивКораблей = Новый Массив;
	
	
	
	Если СостояниеDraftOption["PlayerId"] = 0 Тогда 
		Корабликforward = Новый Соответствие;
		Корабликforward.Вставить("Position", "7/0/0");
		Корабликforward.Вставить("CompleteShipId", "forward");
		МассивКораблей.Добавить(Корабликforward);
		
		Корабликforward = Новый Соответствие;
		Корабликforward.Вставить("Position", "11/0/0");
		Корабликforward.Вставить("CompleteShipId", "forward");
		МассивКораблей.Добавить(Корабликforward);
		
		Корабликforward = Новый Соответствие;
		Корабликforward.Вставить("Position", "15/0/0");
		Корабликforward.Вставить("CompleteShipId", "forward");
		МассивКораблей.Добавить(Корабликforward);
		
		Корабликforward = Новый Соответствие;
		Корабликforward.Вставить("Position", "19/0/0");
		Корабликforward.Вставить("CompleteShipId", "forward");
		МассивКораблей.Добавить(Корабликforward);
		
		Корабликforward = Новый Соответствие;
		Корабликforward.Вставить("Position", "23/0/0");
		Корабликforward.Вставить("CompleteShipId", "forward");
		МассивКораблей.Добавить(Корабликforward);
	Иначе 
		Корабликforward = Новый Соответствие;

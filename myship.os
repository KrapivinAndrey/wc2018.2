...
Функция ПодготовитьОтвет()
	...
	Для Каждого Корабль Из Корабли Цикл
		...
		ЦельАтакиИндивидуальная = ЦельАтакиИндивидуальная(Корабль);
		Если ЦельАтакиИндивидуальная <> Неопределено Тогда
			Отладка.Добавить("_iat_" + ЦельАтакиИндивидуальная["Id"]);
			ЦельАтаки = ЦельАтакиИндивидуальная;
		КонецЕсли;
...
Функция ЦельАтакиИндивидуальная(Корабль)
	...
	Для Каждого Строка Из ТЗКорабли Цикл
		...
		Если ПолучитьРазрешенныйВыстрел(Корабль, Строка.ИзначальноеСоответствие) <> Неопределено Тогда
			Строка.Доступность = Строка.Доступность + 1;
		КонецЕсли;

	КонецЦикла;

	ТЗКорабли.Сортировать("Доступность УБЫВ, Скорость ВОЗР");

	Если ТЗКорабли[0].Доступность = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;

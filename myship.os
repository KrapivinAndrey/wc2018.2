...
Функция ПодготовитьОтвет()
	...
	// Если противник разряжен, используем фокус
	...
	Для Каждого Корабль Из Корабли Цикл
    	...
    	Если РасстояниеМеждуКораблямиПротивника > 30 Тогда
		...
		Иначе
			...
			Если ПолучитьРазрешенныйВыстрел(Корабль, ЦельПеремещения) <> Неопределено Тогда
				ДобавитьКоманду(Ответ, Автопилот(Корабль, ПозицияКОтступлениюПоНачальнойПозиции(Корабль)));		
			Иначе		
				ДобавитьКоманду(Ответ, Автопилот(Корабль, ЦельПеремещения["Position"]));
			КонецЕсли;

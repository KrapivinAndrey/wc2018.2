...
Функция Таран()
	...
	Для Каждого КорабльПротивника Из КораблиПротивника Цикл
		Для Каждого КорабльПротивника2 Из КораблиПротивника Цикл
			СреднееРасстояние = СреднееРасстояние + РасстояниеЧебышев(
				КорабльПротивника["Position"], КорабльПротивника2["Position"]);
		КонецЦикла;
	КонецЦикла;
	...
	Если СреднееРасстояние > 70 Тогда
...
Функция ПолучитьВекторУскоренияКТочке(Корабль, Координаты)
	...
	Пока i <= МаксимальноеУскорение Цикл
		...
		Если (КоординатыКорабля.X > X_Цели И Координата_X > X_Цели)
			...
			Если НЕ Вылетим(Координата_X, СкоростьКорабля.X + ВекторУскорения.X * i) Тогда
		...
		Если (КоординатыКорабля.Y > Y_Цели И Координата_Y > Y_Цели)
			...
			Если НЕ Вылетим(Координата_Y, СкоростьКорабля.Y + ВекторУскорения.Y * i) Тогда
		...
		Если (КоординатыКорабля.Z > Z_Цели И Координата_Z > Z_Цели)
			...
			Если НЕ Вылетим(Координата_Z, СкоростьКорабля.Z + ВекторУскорения.Z * i) Тогда
				Z = ВекторУскорения.Z * i;
			КонецЕсли;
		КонецЕсли;
		
		Если Координата_X = X_Цели И НЕ Вылетим(Координата_X, СкоростьКорабля.X + ВекторУскорения.X * i) Тогда
			X = ВекторУскорения.X * i;
		КонецЕсли;
		Если  Координата_Y = Y_Цели И НЕ Вылетим(Координата_Y, СкоростьКорабля.Y + ВекторУскорения.Y * i) Тогда
			Y = ВекторУскорения.Y * i;
		КонецЕсли;
		Если  Координата_Z = Z_Цели И НЕ Вылетим(Координата_Z, СкоростьКорабля.Z + ВекторУскорения.Z * i) Тогда
			Z = ВекторУскорения.Z * i;
		КонецЕсли;
	...
	Если КритичноеУскорение(Корабль,ВекторМаксимальногоТорможения) Тогда 
		ВекторМаксимальногоТорможения = ПолучитьВекторДляОстановкиКорабля(Корабль);
	КонецЕсли;
...
Функция Вылетим(Координата, Скорость)
	
	Если Скорость > 0 Тогда 
		
		Точка = Координата + Скорость - 2; 
	Иначе 
		Точка = Координата + Скорость + 2;
	КонецЕсли;
	
	
	Возврат ((Точка < 0) ИЛИ (Точка > 28)); 

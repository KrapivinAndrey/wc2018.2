...
Процедура КольцоСмерти_Призраки()
	...
	Враг = ОпределитьБлижайшегоВрагаПоОсиНаСледующемХоду(КорабльПризрак, "Y");
	
	// Если КоординатыНаходятсяВКубе(ПолучитьВекторКоординатКорабля(Враг)) = 1 ;
	
	// Если СкоплениеВраговБольшеВКубе() 
...
Функция ПостроитсяНаПлоскости_ОтносительноТочки(ВекторКоординаты)
	КорабльПризрак = глДопПеременная1;
	КоординатыПризрака = ПолучитьВекторКоординатКорабля(КорабльПризрак);
	ПерпендикулярныйВектор = НовыйВектор(ВекторКоординаты.X - КоординатыПризрака.X, ВекторКоординаты.Y - КоординатыПризрака.Y, ВекторКоординаты.Z - КоординатыПризрака.Z);

	//  ПерпендикулярныйВектор.X * ( x - КоординатыПризрака.X) + ПерпендикулярныйВектор.Y * ( y - КоординатыПризрака.Y) + ПерпендикулярныйВектор.Z * ( z - КоординатыПризрака.Z)
	

	// Для каждого Корабль из Корабли Цикл 
	// 	Если ПорядковыйНомерКорабля(Корабль) = 0 Тогда 
	// 		Z = -ПерпендикулярныйВектор.X * (- 2) - ПерпендикулярныйВектор.Y * (-)
...
Функция СкоплениеВраговБольшеВКубе(); // На Ход Вперед 
	...
	Функция ОпределитьБлижайшегоВрагаПоОсиНаСледующемХоду(Корабль, ОсьСтрокой);
		
		min = РазмерКарты; 
		ИскомыйВраг = Неопределено;
		
		Для каждого Враг из КораблиПротивника Цикл 
			КоординатыПротивника = ПолучитьВекторКоординатКорабляСУчетомСкоростиИВектораУскорения(Корабль);
...
Процедура ДрафтКольцоСмерти_Призраки()
	...
	КорабльПризрак = Новый Соответствие;
	КорабльПризрак.Вставить("Velocity", "0/0/0");
	КорабльПризрак.Вставить("Position", "13/"+КоординатаY+"/12");
	
	глДопПеременная1 = КорабльПризрак;	
КонецПроцедуры

...
Перем РазмерКарты;
Перем ЭтоДрафт;
Перем глДопПеременная1;
Перем глДопПеременная2;
Перем глДопПеременная3;
Перем PlayerId;
/////////////////////////////////////////////////////
// Сценарий
...
Процедура ВыполнитьСценарий()
	...
	КольцоСмерти_Призраки();
	Отстреляться();
	
КонецПроцедуры

Процедура ВыполнитьСценарийДрафт()
	ДрафтКольцоСмерти_Призраки();
	// ДрафтДляЗмеи();
...
Функция ПолучитьРазрешенныйВыстрел(Корабль, Враг, Знач Орудие = Неопределено)
	...
	ДружественныйОгонь = ДружественныйОгонь(Корабль, ОжидаемоеПоложениеВрага, Орудие);
...
Функция ДружественныйОгонь(Корабль, Цель, Орудие)
	...
	ТочкиЛучаПоБрезенхему = ТочкиЛучаПоБрезенхему(ТочкаНачалаВыстрела, Цель,, Орудие["Radius"]);
...
Функция Змея()
	Для каждого Корабль из Корабли Цикл 
		...
		Если ВекторСкоростиКорабля.Y > 0 Тогда 
			Летим(Корабль, НовыйВектор(ВекторКоординатКорабля.X , РазмерКартыДляКорабля , ВекторКоординатКорабля.Z));
		КонецЕсли;
		Если ВекторСкоростиКорабля.Y < 0 Тогда 
			Летим(Корабль, НовыйВектор(ВекторКоординатКорабля.X , 0 , ВекторКоординатКорабля.Z));
		КонецЕсли;	
		
		Если ВекторКоординатКорабля.Y = РазмерКартыДляКорабля И ВекторСкоростиКорабля.Y = 0 Тогда 
			
			Летим(Корабль, НовыйВектор(ВекторКоординатКорабля.X , 0 , ВекторКоординатКорабля.Z));
			
		КонецЕсли; 
		
		Если ВекторКоординатКорабля.Y = 0 И ВекторСкоростиКорабля.Y = 0 Тогда  
...
Процедура КольцоСмерти_Призраки()

	Враг = ОпределитьБлижайшегоВрагаПоОсиНаСледующемХоду("Y");
	
	КоординатыВрага = ПолучитьВекторКоординатКорабля(Враг);
	
	X = КоординатыВрага.X;
	Если PlayerId = 0 Тогда 
		Y = КоординатыВрага.Y - 6 ; // Расстояние выстрела 
	иначе 
		Y = КоординатыВрага.Y + 6 ; // Расстояние выстрела 	
	КонецЕсли;
	Z = КоординатыВрага.Z;
	...
	ПостроитсяНаПлоскости_ОтносительноТочки(Строка(X) + "/" + Строка(Y) + "/" + Строка(Z));
	
	
	
КонецПроцедуры

// Вектор от A до B  = X_B - X_A ....
// Уровнение плоскости проходящей через точку (X_T, Y_T, Z_T) перпендикулярно вектору (X_V, Y_V, Z_V) 
// X_V * (x - X_T) + Y_V * (y - Y_T) + Z_V (z - Z_T) ; 
// ПерпендикулярныйВектор = НовыйВектор(КоординатыПризрака.X - ВекторКоординаты.X,  КоординатыПризрака.Y - ВекторКоординаты.Y, КоординатыПризрака.Z - ВекторКоординаты.Z);
//  ПерпендикулярныйВектор.X * ( x - КоординатыПризрака.X) + ПерпендикулярныйВектор.Y * ( y - КоординатыПризрака.Y) + ПерпендикулярныйВектор.Z * ( z - КоординатыПризрака.Z) = 0 Уравнение плоскости

Функция ПостроитсяНаПлоскости_ОтносительноТочки(Точка)
	

	КоординатыПризрака = НовыйВектор(Точка);
	
	Для каждого Корабль из Корабли Цикл 
		Если ПорядковыйНомерКорабля(Корабль) = 0 Тогда 
			X = КоординатыПризрака.X - 2 ;
			Z = КоординатыПризрака.Z - 4 ;
			Y = КоординатыПризрака.Y ;
			
			Летим(Корабль, НовыйВектор(X,Y,Z), Ложь);
			
		КонецЕсли;
		
		Если ПорядковыйНомерКорабля(Корабль) = 1 Тогда 
			X = КоординатыПризрака.X + 2 ;
			Z = КоординатыПризрака.Z - 4 ;
			Y = КоординатыПризрака.Y ;
		...
		КонецЕсли;	
		Если ПорядковыйНомерКорабля(Корабль) = 2 Тогда 
			X = КоординатыПризрака.X - 4 ;
			Z = КоординатыПризрака.Z ;
			Y = КоординатыПризрака.Y ;
			
			Летим(Корабль, НовыйВектор(X,Y,Z), Ложь);
			
		КонецЕсли;	
		Если ПорядковыйНомерКорабля(Корабль) = 3 Тогда 
			X = КоординатыПризрака.X - 2 ;
			Z = КоординатыПризрака.Z + 4 ;
			Y = КоординатыПризрака.Y ;
		...
		КонецЕсли;	
		Если ПорядковыйНомерКорабля(Корабль) = 4 Тогда 
			X = КоординатыПризрака.X + 2 ;
			Z = КоординатыПризрака.Z + 4 ;
			Y = КоординатыПризрака.Y ;
		...
		КонецЕсли;	
		Если ПорядковыйНомерКорабля(Корабль) = 5 Тогда 
			X = КоординатыПризрака.X + 4 ;
			Z = КоординатыПризрака.Z ;
			Y = КоординатыПризрака.Y ;
...
Функция СтройОтносительноВектора(ВекторКоэфицентовЛинииПостроения)
	
	
	
	
	
	КорабльПризрак = глДопПеременная1;
...
Функция СкоплениеВраговБольшеВКубе(); // На Ход Вперед 
	
	МассивКубов = Новый Соответствие;
	НомерКуба = 1;
	Пока НомерКуба <= 27 Цикл 
		
		МассивКубов.Вставить("Куб" + НомерКуба, 0);
		НомерКуба = НомерКуба + 1;
	КонецЦикла;
	
	Для каждого Враг из КораблиПротивника Цикл 
		
		НомерКуба = КорабльБудетВКубе(Враг);
		НомерКубаСтрокой = Строка(НомерКуба);
		
		МассивКубов.Вставить("Куб" + НомерКубаСтрокой, МассивКубов["Куб" + НомерКубаСтрокой] + 1);
		
	КонецЦикла;
	
	МаксимальноеКоличествоВрагов = 0; 
	НомерКуба = 0;
	
	Пока НомерКуба < 27 Цикл 
		НомерКуба = НомерКуба + 1;
		КоличествоВраговВКубе = МассивКубов["Куб" + НомерКуба];
		
		Если МассивКубов["Куб" + НомерКуба] > МаксимальноеКоличествоВрагов Тогда 
			МаксимальноеКоличествоВрагов = МассивКубов["Куб" + НомерКуба];
		КонецЕсли;
...
Функция КоординатыНаходятсяВКубе(Координаты);
	
	Если Координаты.X <= 10                       И Координаты.Y <= 10 						И Координаты.Z <=10 Тогда
		Возврат 1 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И Координаты.Y <= 10 						И Координаты.Z <=10 Тогда
		Возврат 2
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И Координаты.Y <= 10 						И Координаты.Z <=10 Тогда
		Возврат 3 
	КонецЕсли;
	Если Координаты.X <= 10                       И (Координаты.Y > 10 И Координаты.Y <=20) И Координаты.Z <=10 Тогда 
		Возврат 4 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И (Координаты.Y > 10 И Координаты.Y <=20) И Координаты.Z <=10 Тогда 
		Возврат 5 
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И (Координаты.Y > 10 И Координаты.Y <=20) И Координаты.Z <=10 Тогда 
		Возврат 6 
	КонецЕсли;
	Если Координаты.X <= 10                       И (Координаты.Y > 20 И Координаты.Y <=28) И Координаты.Z <=10 Тогда 
		Возврат 7 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И (Координаты.Y > 20 И Координаты.Y <=28) И Координаты.Z <=10 Тогда 
		Возврат 8
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И (Координаты.Y > 20 И Координаты.Y <=28) И Координаты.Z <=10 Тогда 
		Возврат 9
	КонецЕсли;
	Если Координаты.X <= 10                       И Координаты.Y <= 10 						И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда
		Возврат 10 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И Координаты.Y <= 10 						И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда
		Возврат 11
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И Координаты.Y <= 10 						И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда
		Возврат 12 
	КонецЕсли;
	Если Координаты.X <= 10                       И (Координаты.Y > 10 И Координаты.Y <=20) И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда 
		Возврат 13 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И (Координаты.Y > 10 И Координаты.Y <=20) И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда 
		Возврат 14 
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И (Координаты.Y > 10 И Координаты.Y <=20) И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда 
		Возврат 15 
	КонецЕсли;
	Если Координаты.X <= 10                       И (Координаты.Y > 20 И Координаты.Y <=28) И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда 
		Возврат 16 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И (Координаты.Y > 20 И Координаты.Y <=28) И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда 
		Возврат 17
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И (Координаты.Y > 20 И Координаты.Y <=28) И (Координаты.Z > 10 И Координаты.Z <=20 ) Тогда 
		Возврат 18
	КонецЕсли;
	Если Координаты.X <= 10                       И Координаты.Y <= 10 						И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 19 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И Координаты.Y <= 10 						И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 20
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И Координаты.Y <= 10 						И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 21 
	КонецЕсли;
	Если Координаты.X <= 10                       И (Координаты.Y > 10 И Координаты.Y <=20) И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 22 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И (Координаты.Y > 10 И Координаты.Y <=20) И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда 
		Возврат 23 
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И (Координаты.Y > 10 И Координаты.Y <=20) И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 24 
	КонецЕсли;
	Если Координаты.X <= 10                       И (Координаты.Y > 20 И Координаты.Y <=28) И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 25 
	КонецЕсли;
	Если (Координаты.X > 10 И Координаты.X <= 20) И (Координаты.Y > 20 И Координаты.Y <=28) И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 26
	КонецЕсли;
	Если (Координаты.X > 20 И Координаты.X <= 28) И (Координаты.Y > 20 И Координаты.Y <=28) И (Координаты.Z > 20 И Координаты.Z <= 28 ) Тогда
		Возврат 27
	КонецЕсли;
КонецФункции

Функция КорабльБудетВКубе(Корабль, ВекторУскорения = Неопределено);
	
	Возврат КоординатыНаходятсяВКубе(ПолучитьВекторКоординатКорабляСУчетомСкоростиИВектораУскорения(Корабль, ВекторУскорения));
	
КонецФункции

Функция ОпределитьБлижайшегоВрагаПоОсиНаСледующемХоду(ОсьСтрокой);
	...
	Для каждого Враг из КораблиПротивника Цикл 
		КоординатыПротивника = ПолучитьВекторКоординатКорабляСУчетомСкоростиИВектораУскорения(Враг);
		Если PlayerId = 0 Тогда 
			Если КоординатыПротивника[ОсьСтрокой] <= min Тогда
				min = КоординатыПротивника[ОсьСтрокой];
				ИскомыйВраг = Враг;			
			КонецЕсли;
		Иначе 
			Если КоординатыПротивника[ОсьСтрокой] >= max Тогда
				max = КоординатыПротивника[ОсьСтрокой];
				ИскомыйВраг = Враг;			
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьВекторКоординатКорабляСУчетомСкоростиИВектораУскорения(Корабль, ВекторУскорения = Неопределено)
	Координаты = ПолучитьВекторКоординатКорабля(Корабль);
	ВекторСкорости = ПолучитьВекторСкоростиКорабля(Корабль);
	X = Координаты.X + ВекторСкорости.X;
	Y = Координаты.Y + ВекторСкорости.Y;
	Z = Координаты.Z + ВекторСкорости.Z;
	
	Если ВекторУскорения = Неопределено Тогда 
		Возврат НовыйВектор(X,Y,Z);
	Иначе 
		X = X + ВекторУскорения.X;
		Y = Y + ВекторУскорения.Y;
		Z = Z + ВекторУскорения.Z;
		Возврат НовыйВектор(X,Y,Z);
	КонецЕсли;
КонецФункции
/////////////////////////////////////////////////////
...
Процедура ДрафтДляЗмеи()
	...
	ДобавитьКоманду(
	ВыборКорабля("forward", "7/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("forward", "11/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("forward", "15/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("forward", "19/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("forward", "23/"+КоординатаY+"/"+КоординатаZ));
	
	
КонецПроцедуры

Процедура ДрафтКольцоСмерти_Призраки()
	

	РазмерКарты 	= Состояние["MapSize"];
	PlayerId = Число(Состояние["PlayerId"]);	
	...
	ДобавитьКоманду(
	ВыборКорабля("scout", "11/"+КоординатаY+"/8"));
	
	ДобавитьКоманду(
	ВыборКорабля("scout", "15/"+КоординатаY+"/8"));
	
	ДобавитьКоманду(
	ВыборКорабля("scout", "9/"+КоординатаY+"/12"));
	
	ДобавитьКоманду(
	ВыборКорабля("scout", "11/"+КоординатаY+"/16"));
	
	ДобавитьКоманду(
	ВыборКорабля("scout", "15/"+КоординатаY+"/16"));
	
	ДобавитьКоманду(
	ВыборКорабля("scout", "17/"+КоординатаY+"/12"));
...
Функция ПорядковыйНомерКорабля(Корабль)
...
Функция РасстояниеМеждуКорабльИПозиция(Знач ПозицияКорабля, Знач Позиция, Чебышев = Истина)
	...
	Для Каждого ЯчейкаКорабля Из МассивКоординатКорабля Цикл
		...
		Иначе
			НоваяСтрока.Расстояние = РасстояниеМанхэттен(ЯчейкаКорабля, Позиция);
		КонецЕсли;
...
Функция ВыборКорабля(Знач CompleteShipId = "scout", Знач Position = Неопределено)

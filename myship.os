...
Функция ПодготовитьОтвет()
	...
	ДобавитьСообщение(Ответ, ПарсерJSON.записатьJSON(ОбщаяЦельАтаки));
...
Функция ПолучитьРазрешенныйВыстрел(Орудие, Корабль, Враг)
	...
	ПоправкаНаСближение = 2;
	ЦельВРадиусеПоражения = Орудие["Radius"] + ПоправкаНаСближение >= РасстояниеЧебышев(Корабль["Position"], Враг["Position"]);

	ОжидаемоеПоложениеВрага = СложитьВекторы(Враг["Position"], Враг["Velocity"]); 
...
Функция СложитьВекторы(Вектор1, Вектор2)

...
Процедура ВыполнитьСценарий()
	...
	В1 = Выстрел(Корабль, ПолучитьВекторКоординатКорабля(БлижайшийВраг));
	В2 = Выстрел(Корабль, ПолучитьВекторКоординатКорабля(БлижайшийВраг));
	
	ДобавитьКоманду(В1);
	ДобавитьКоманду(В2);11
...
// 		КомандаПеремещения = Летим(Корабль, НачальнаяПозицияКорабля(Корабль), Ложь);
// 	Иначе				
// 		КомандаПеремещения = Летим(Корабль, ЦельПеремещения["Position"], Ложь);
// 	КонецЕсли;
...
Процедура Отстреляться()
	...
	Для Каждого Корабль Из Корабли Цикл
		...
		Выстрел = Выстрел(Корабль, СценарииСтрельбыКорабля[0].ВыстрелТочка);
		Выстрел1 = Выстрел(Корабль, СценарииСтрельбыКорабля[0].ВыстрелТочка);
		
		ДобавитьКоманду(Выстрел);
		ДобавитьКоманду(Выстрел1);

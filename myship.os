...
Функция НовыйПараметрыУскорения(Корабль, КоординатыЦели)
	...
	X_Корабля = НовыйВектор(Корабль["Position"]).X;
	Y_Корабля = НовыйВектор(Корабль["Position"]).Y;
	Z_Корабля = НовыйВектор(Корабль["Position"]).Z;
	
	X_СкоростьКорабля = НовыйВектор(Корабль["Velocity"]).X;
	Y_СкоростьКорабля = НовыйВектор(Корабль["Velocity"]).Y;
	Z_СкоростьКорабля = НовыйВектор(Корабль["Velocity"]).Z;
	
	Двигатель = ДвигательКорабля(Корабль);
	МаксимальноеУскорение = Двигатель["MaxAccelerate"];
	
	X_Цели = НовыйВектор(КоординатыЦели).X;
	Y_Цели = НовыйВектор(КоординатыЦели).Y;
	Z_Цели = НовыйВектор(КоординатыЦели).Z;
	...
	Пока i <= МаксимальноеУскорение Цикл
		...
		Если X_Корабля < X_Цели Тогда
			Если X_Корабля + ТормознойПуть(X_СкоростьКорабля + i) < X_Цели Тогда
				x = i;
			КонецЕсли;
		Иначе
			Если X_Корабля + ТормознойПуть(X_СкоростьКорабля - i) > X_Цели Тогда
		...
		Если Y_Корабля < Y_Цели Тогда
			Если Y_Корабля + ТормознойПуть(Y_СкоростьКорабля + i) < Y_Цели Тогда
				y = i;
			КонецЕсли;
		Иначе
			Если Y_Корабля + ТормознойПуть(Y_СкоростьКорабля - i) > Y_Цели Тогда
		...
		Если Z_Корабля < Z_Цели Тогда
			Если Z_Корабля + ТормознойПуть(Z_СкоростьКорабля + i) < Z_Цели Тогда
				z = i;
			КонецЕсли;
		Иначе
			Если Z_Корабля + ТормознойПуть(Z_СкоростьКорабля - i) > Z_Цели Тогда
	...
	Если x = Неопределено Тогда 
		Если X_Корабля < X_Цели Тогда 
			Если X_Корабля + ТормознойПуть(X_СкоростьКорабля) < X_Цели Тогда 
				x = 0;
			КонецЕсли;
		Иначе 
			Если X_Корабля + ТормознойПуть(X_СкоростьКорабля) > X_Цели Тогда 
	...
	Если y = Неопределено Тогда 
		Если Y_Корабля < Y_Цели Тогда 
			Если Y_Корабля + ТормознойПуть(Y_СкоростьКорабля) < Y_Цели Тогда 
				y = 0;
			КонецЕсли;
		Иначе 
			Если Y_Корабля + ТормознойПуть(Y_СкоростьКорабля) > Y_Цели Тогда 
	...
	Если z = Неопределено Тогда 
		Если Z_Корабля < Z_Цели Тогда 
			Если Z_Корабля + ТормознойПуть(Z_СкоростьКорабля) < Z_Цели Тогда 
				z = 0;
			КонецЕсли;
		Иначе 
			Если Z_Корабля + ТормознойПуть(Z_СкоростьКорабля) > Z_Цели Тогда 

...
Процедура ДрафтПоУмолчанию()
	...
	Пока СтоимостьДрафта > ВсегоДенег Цикл
	
		Если Количество <> 0 Тогда	
			Количество       = Количество - КоличествоСкаут;
			КоличествоСкаут  = КоличествоСкаут + 1;	
		ИначеЕсли Количество = 0 
			И КоличествоСкаут = 0 Тогда
			ВызватьИсключение "shit draft";
		Иначе
			КоличествоСкаут  = КоличествоСкаут - 1;
		КонецЕсли;
	...
	НужноРасставитьКораблей = Количество + КоличествоСкаут;
	
	Пока НужноРасставитьКораблей <> 0 Цикл

		НужноРасставитьКораблей = НужноРасставитьКораблей - 1;
		
		ВыборКорабля = Неопределено;

		Если Количество > 0 Тогда
			
			Количество    = Количество - 1;
			ВыборКорабля = ВыборКорабля(Корабль["Id"]);

		ИначеЕсли КоличествоСкаут > 0 Тогда

			КоличествоСкаут = КоличествоСкаут - 1;
			ВыборКорабля    = ВыборКорабля(КорабльСкаут["Id"]);
		
		КонецЕсли;

		Если ВыборКорабля <> Неопределено Тогда

			НужноРасставитьКораблей = НужноРасставитьКораблей - 1;
			ДобавитьКоманду(ВыборКорабля);

...
Функция Мушки(Корабли, Ответ)

	ГСЧ = Новый ГенераторСлучайныхЧисел();
	ДобавитьСообщение(Ответ, "Мушки");
	...
	Для каждого Корабль из Корабли Цикл 
		...
		РастояниеДодальней = Мин(Позиция.X, Позиция.Y, Позиция.Z);
		ДобавитьСообщение(Ответ, "РастояниеДодальней");

		Если РастояниеДодальней = Позиция.X Тогда 

			Позиция.X = 30;
			Позиция.Y = ГСЧ.СлучайноеЧисло(0,30);
			Позиция.Z = ГСЧ.СлучайноеЧисло(0,30);
			ВекторДвижения = Позиция;
			
			ДобавитьКоманду(Ответ, Ускорение(Корабль, ВекторДвижения));	
			ДобавитьСообщение(Ответ, "ACCELERATE");
			Продолжить;
		КонецЕсли;

		Если РастояниеДодальней = Позиция.Y Тогда 

			Позиция.X = ГСЧ.СлучайноеЧисло(0,30);
			Позиция.Y = 30;
			Позиция.Z = ГСЧ.СлучайноеЧисло(0,30);
		...
		Если РастояниеДодальней = Позиция.Z Тогда 

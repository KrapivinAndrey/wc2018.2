...
Процедура ВыполнитьСценарийДрафт()
	...
	СборкаКорабля 		= СобратьКорбль("big_health");
	СборкаКорабляРельса = СобратьКорбль("big_health, big_railgun, big_energy, big_energy, big_energy");	

	// Глаза

	ВыбратьКорабль(NULL, "6/"+Y+"/12", СборкаКорабляРельса["Equipment"]);
	ВыбратьКорабль(NULL, "6/"+Y+"/20", СборкаКорабляРельса["Equipment"]);
	
	// нос

	ВыбратьКорабль(NULL, "10/"+Y+"/16", СборкаКорабля["Equipment"]);
	...
	ВыбратьКорабль(NULL, "12/"+Y+"/10", СборкаКорабля["Equipment"]);
	ВыбратьКорабль(NULL, "12/"+Y+"/22", СборкаКорабля["Equipment"]);

	// линия 2
	
	ВыбратьКорабль(NULL, "14/"+Y+"/12", СборкаКорабля["Equipment"]);
	ВыбратьКорабль(NULL, "14/"+Y+"/14", СборкаКорабля["Equipment"]);
	ВыбратьКорабль(NULL, "14/"+Y+"/18", СборкаКорабля["Equipment"]);
	ВыбратьКорабль(NULL, "14/"+Y+"/20", СборкаКорабля["Equipment"]);

	// линия 3

	ВыбратьКорабль(NULL, "16/"+Y+"/16", СборкаКорабля["Equipment"]);
...
Функция СобратьКорбль(СтрокаМодули = "big_energy, big_health, big_blaster, big_engine")
...
Функция Змея()
	...
	Если НомерИтерации > 800
		ИЛИ КораблиПротивника.Количество() = 3 Тогда	

		Радиус = 6;
		Если КораблиПротивника.Количество() = 3 Тогда
			Радиус = 3;
		КонецЕсли;
		...
		X = КоординатыВрага.X;
		Если PlayerId = 0 Тогда 
			Y = КоординатыВрага.Y - Радиус ; // Расстояние выстрела 
		иначе 
			Y = КоординатыВрага.Y + Радиус ; // Расстояние выстрела 	
		КонецЕсли;
		Z = КоординатыВрага.Z;
	...
	ПостроитсяНаПлоскости_ОтносительноТочки("" + X + "/" + Y + "/" + Z);
...
Процедура ДрафтДляЗмеи()
	...
	СборкаКорабля = СобратьКорбль("big_energy, big_health, big_health, giant_blaster, big_engine, big_heal");
	...
	ВыбратьКорабль(СборкаКорабля["Id"], "7/"  + КоординатаY+"/"+КоординатаZ, СборкаКорабля["Equipment"]);
	ВыбратьКорабль(СборкаКорабля["Id"], "10/"  +КоординатаY+"/"+КоординатаZ, СборкаКорабля["Equipment"]);
	ВыбратьКорабль(СборкаКорабля["Id"], "13/" + КоординатаY+"/"+КоординатаZ, СборкаКорабля["Equipment"]);
	ВыбратьКорабль(СборкаКорабля["Id"], "16/" + КоординатаY+"/"+КоординатаZ, СборкаКорабля["Equipment"]);
	ВыбратьКорабль(СборкаКорабля["Id"], "19/" + КоординатаY+"/"+КоординатаZ, СборкаКорабля["Equipment"]);
...
Процедура Отстреляться()
	...
	ПриоритетныеЦели.Добавить(КораблиПротивника[0]);
...
Функция ПолучитьВекторУскоренияКТочке(Корабль, Координаты, Минимально = Ложь)
	...
	Если Минимально Тогда 
		Если Не (МодульЧисла(СкоростьКорабля.X) > 1) Тогда 
			Если МодульЧисла(СкоростьКорабля.X + X) > 1 Тогда 
				X = 0 ;
			КонецЕсли
		КонецЕсли;
		Если Не (МодульЧисла(СкоростьКорабля.Y) > 1) Тогда 
			Если МодульЧисла(СкоростьКорабля.Y + Y) > 1 Тогда 
				Y = 0 ;
			КонецЕсли
		КонецЕсли;
		Если Не (МодульЧисла(СкоростьКорабля.Z) > 1) Тогда 
			Если МодульЧисла(СкоростьКорабля.Z + Z) > 1 Тогда 
				Z = 0 ;
			КонецЕсли
		КонецЕсли;		
	КонецЕсли;
...
Функция ПолучитьВекторМинимальногоУскоренияКТочке(Корабль, Точка)
	КорабльНаш 			    = КорабльНаш(Корабль);
	КоординатыКорабля 	    = ПолучитьВекторКоординатКорабля(Корабль);
	Скорость		    	= ПолучитьВекторСкоростиКорабля(Корабль);
	МаксимальноеУскорение   = ?(КорабльНаш, МаксимальноеУскорениеНашегоКорабля(Корабль), МаксимальноеУскорениеВражескогоКорабля(Корабль));
	КоординатыТочки = НовыйВектор(Точка);
...
Функция Летим(Корабль, Координаты, Максимально = Ложь, Минимально = Ложь)
	...
	Если Максимально  Тогда
		ВекторУскорения = ПолучитьМаксимальныйВекторУскорения(Корабль, КоординатыЦели);
	ИначеЕсли Минимально Тогда 
		ВекторУскорения = ПолучитьВекторУскоренияКТочке(Корабль, КоординатыЦели, Истина);
	Иначе 

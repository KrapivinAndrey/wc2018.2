...
Функция ПодготовитьОтвет()
	...
	Для Каждого Корабль Из Корабли Цикл
		...
		Если ЦельДоступна Тогда
			...
			ДобавитьКоманду(Ответ, Ускорение(Корабль, ПозицияКОтступлению));
			
		Иначе		
			ДобавитьКоманду(Ответ, Ускорение(Корабль, ОбщаяЦельАтаки["Position"]));
		КонецЕсли;
...
Функция НовыйПараметрыУскорения(Корабль, КоординатыЦели)
	...
	Если Число(Корабль["Id"]) = 0 Тогда
		Отладка.Добавить("X_K: "+ Строка(X_Корабля) + "; C: " + Строка(X_Цели) + "/" + Строка(Y_Цели) + "/" + Строка(Z_Цели) + "; X_V: " + Строка(X_СкоростьКорабля));
	КонецЕсли;

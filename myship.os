...
Процедура ДрафтДляЗмеи()
	...
	ДобавитьКоманду(
	ВыборКорабля("daedalus", "7/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("daedalus", "9/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("daedalus", "11/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("daedalus", "13/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
	ВыборКорабля("daedalus", "15/"+КоординатаY+"/"+КоординатаZ));
...
Процедура Отстреляться()
	...
	Для Каждого Корабль Из Корабли Цикл
		Для Каждого Орудие Из ОрудияКорабля(Корабль) Цикл
			Если Число(Орудие["Type"]) = 7 Тогда 	 
				ВекторУскоренияСущности = глДопПеременная1;
				Кородинаты = ПолучитьВекторКоординатКорабля(Корабль);
				Скорость = ПолучитьВекторСкоростиКорабля(Корабль);
				X = Кородинаты.X + Скорость.X + ВекторУскоренияСущности.X;
				Y = Кородинаты.Y + Скорость.Y + ВекторУскоренияСущности.Y;
				Z = Кородинаты.Z + Скорость.Z + ВекторУскоренияСущности.Z;
				
				Если ПолучитьПорядковыйНомер(Корабль) = 0 Тогда 
					Выстрел = Выстрел(Корабль, НовыйВектор(X + 10,Y,Z), Орудие);
				КонецЕсли;
				Если ПолучитьПорядковыйНомер(Корабль) = 1 Тогда 
					Выстрел = Выстрел(Корабль, НовыйВектор(X + 8,Y,Z), Орудие);
				КонецЕсли;
				Если ПолучитьПорядковыйНомер(Корабль) = 2 Тогда 
					minHP = 100;
					КорабликКоцаный = Неопределено;
					Для каждого Кораблик из Корабли Цикл
						Если НЕ ПолучитьПорядковыйНомер(Корабль) = 2  Тогда
							Если Кораблик["Health"] <= minHP Тогда
								minHP = Кораблик["Health"];
								КорабликКоцаный = Кораблик;							
							КонецЕсли;
						КонецЕсли;
					КонецЦикла;
					Если КорабликКоцаный = Неопределено Тогда
						КорабликКоцаный = Корабли[0];
					КонецЕсли;
					Если ПолучитьПорядковыйНомер(КорабликКоцаный) > 2 Тогда 
						Выстрел = Выстрел(Корабль, НовыйВектор(X + 6,Y,Z), Орудие);
					Иначе
						Выстрел = Выстрел(Корабль, НовыйВектор(X - 4,Y,Z), Орудие);
					КонецЕсли;
					
				КонецЕсли;
				Если ПолучитьПорядковыйНомер(Корабль) = 3 Тогда 
					Выстрел = Выстрел(Корабль, НовыйВектор(X - 6,Y,Z), Орудие);
				КонецЕсли;
				Если ПолучитьПорядковыйНомер(Корабль) = 4 Тогда 
					Выстрел = Выстрел(Корабль, НовыйВектор(X - 8,Y,Z), Орудие);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого Орудие Из ОрудияКорабля(Корабль) Цикл
			Если НЕ Число(Орудие["Type"]) = 7 Тогда 
...
Функция ПолучитьПорядковыйНомер(Корабль) 
...
Функция Летим(Корабль, Координаты, Максимально = Ложь)
	...
	глДопПеременная1 = ВекторУскорения;
КонецФункции

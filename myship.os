...
Функция Таран()
	...
	Для каждого Корабль из Корабли Цикл 
		...
		Если НомерИтерацииВылета <= НомерИтерации Тогда 
			...
			Летим(Корабль, ПредпологаемаяТочкаВрага , Ложь);
		КонецЕсли;
	
		Команда = Новый Структура("Command, Parameters");	
		Команда.Command = "SCAN";
		Команда.Parameters = Новый Структура("Id, Name", Корабль["Id"], "big_radar");		

		ДобавитьКоманду(Команда);

	КонецЦикла;
	
	Отладка.Добавить(ПарсерJSON.записатьJSON(КораблиПротивника[0]));
...
Процедура ДрафтТаран()
	...
	СборкаКорабля =  СобраныйКорабль("Orville");
	...
	ВыбратьКорабль(СборкаКорабля["Id"], "0/"  + КоординатаY+"/"+КоординатаZ);
	ВыбратьКорабль(СборкаКорабля["Id"], "3/"  + КоординатаY+"/"+КоординатаZ);
	ВыбратьКорабль(СборкаКорабля["Id"], "6/"  + КоординатаY+"/"+КоординатаZ);
	//ВыбратьКорабль(СборкаКорабля["Id"], "9/" + КоординатаY+"/"+КоординатаZ);
	//ВыбратьКорабль(СборкаКорабля["Id"], "12/" + КоординатаY+"/"+КоординатаZ);
	//ВыбратьКорабль(СборкаКорабля["Id"], "15/" + КоординатаY+"/"+КоординатаZ);
	
КонецПроцедуры
Процедура ДрафтПодвижногоКуба()
	
	СборкаКорабля = СобратьКорбль("giant_engine, big_health, big_nose, big_nose, big_nose");
...
Функция ДвигательКорабля(Корабль)
	...
	Для Каждого Элемент Из Оборудование Цикл
		...
		Если Элемент["Type"] = 2 ИЛИ Элемент["Type"] = 3 Тогда
			Возврат Элемент;
		КонецЕсли;
...
Функция Летим(Корабль, Координаты, Максимально = Ложь)
	...
	КонецЕсли;
	Отладка.Добавить(" W: " + ВекторСтрокой(ВекторУскорения));
...
Процедура ВыбратьКорабль(Идентификатор, Позиция, Оборудование = Неопределено)

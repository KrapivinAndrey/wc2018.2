...
Процедура ВыполнитьСценарий()
	...
	//Луч = ТочкиЛучаПоБрезенхему("4/0/1", "3/0/6", Истина);

	ПозицияКорабля = ПозицияКорабля(Корабли[0]);
	Конец 		   = НовыйВектор(НомерИтерации, 29, 16);
	//ПозицияКорабля = НовыйВектор(4,0,0);
	//Конец = НовыйВектор(5, 0, 6);

	Расстояние = РасстояниеМеждуКорабльИПозиция(ПозицияКорабля, Конец, Ложь);

	Луч = ТочкиЛучаПоБрезенхему(Расстояние.ЯчейкаКорабля, Конец, Истина);

	Отладка.Добавить("S:" + ВекторСтрокой(Расстояние.ЯчейкаКорабля));
	Отладка.Добавить("E:" + ВекторСтрокой(Конец));

	Для Каждого Элемент Из Луч Цикл
		Отладка.Добавить("_" + Элемент);
	КонецЦикла;

	ДобавитьКоманду(
		Выстрел(Корабли[0], Конец)
	);

	Если НомерИтерации > 20 Тогда
...
Функция Змея()
	...
	Для каждого Корабль из Корабли Цикл 
		...
		Отладка.Добавить("_" + ПорядокХода);
...
Процедура ДрафтДляЗмеи()
	...
	ДобавитьКоманду(
		ВыборКорабля("forward", "4/"+КоординатаY+"/"+КоординатаZ));
	
	ДобавитьКоманду(
		ВыборКорабля("forward", "8/"+КоординатаY+"/"+КоординатаZ));

	ДобавитьКоманду(
		ВыборКорабля("forward", "12/"+КоординатаY+"/"+КоординатаZ));

	ДобавитьКоманду(
		ВыборКорабля("forward", "16/"+КоординатаY+"/"+КоординатаZ));

	ДобавитьКоманду(
		ВыборКорабля("forward", "20/"+КоординатаY+"/"+КоординатаZ));
...
Функция РасчитатьВыстрелЛуч(Орудие, КорабльПозиция, Точка)
	...
	// Модифцировать точку когда она не на максимальном радиусе
